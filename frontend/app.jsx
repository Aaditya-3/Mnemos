...
function LoginScreen({ mode, setMode, onSubmit, isLoading, error, theme, onThemeToggle }) {
                ...
                <div className="grid grid-cols-2 gap-2 mb-5 bg-[rgb(var(--bg-alt-rgb))] border border-[rgb(var(--border-rgb)/0.4)] p-1 rounded-xl">
                    <button
                        ...
                        className={`py-2 rounded-lg text-sm font-semibold transition-colors ${
                            mode === "login"
                                ? "bg-[rgb(var(--accent-rgb))] text-[rgb(var(--on-accent-rgb))]"
                                : "text-[rgb(var(--accent-rgb))] hover:text-[rgb(var(--text-rgb))]"
                        }`}
                    >
                        Login
                    </button>
...
function TypewriterText({ text, speed = 18, active = true }) {
    ...
    return (
        <span>
            {display}
            <span className={`ml-0.5 inline-block text-[rgb(var(--accent-rgb))] ${done ? "opacity-0" : "opacity-100 animate-pulse"}`}>|</span>
        </span>
    );
}
        ...
        <div className="flex h-screen bg-[rgb(var(--bg-alt-rgb))] text-[rgb(var(--text-rgb))]">
            ...
            {deleteConfirmChatId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm fade-in">
                    <div className="w-full max-w-sm rounded-2xl border border-[rgb(var(--accent-rgb)/0.45)] bg-[rgb(var(--panel-rgb))] p-5 shadow-2xl soft-pop">
                        <h3 className="text-lg font-semibold text-[rgb(var(--text-rgb))]">Delete Chat?</h3>
                        <p className="mt-2 text-sm text-[rgb(var(--accent-rgb))]">
                            This will permanently remove this chat from your sidebar.
                        </p>
                        <div className="mt-5 flex justify-end gap-2">
                            <button
                                onClick={() => setDeleteConfirmChatId(null)}
                                className="rounded-lg border border-[rgb(var(--accent-rgb)/0.4)] bg-[rgb(var(--bg-alt-rgb))] px-4 py-2 text-sm text-[rgb(var(--text-rgb))] transition-colors hover:bg-[rgb(var(--panel-rgb))]"
                            >
                                Cancel
                            </button>
                            <button
